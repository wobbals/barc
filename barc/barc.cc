...
{
  barc->video_tick_time = 1 / config->video_framerate;
  video_mixer_set_width(barc->video_mixer, config->out_width);
  video_mixer_set_height(barc->video_mixer, config->out_height);
